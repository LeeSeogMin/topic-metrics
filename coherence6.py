...
class VAE(nn.Module):
    ...
    def decode(self, z):
        ...
        return torch.sigmoid(self.fc4(h3))  # sigmoid를 사용하여 0과 1 사이의 값 출력
    ...
    for model_type in model_types:
        ...
        try:
            ...
            if model_type == 'BERTopic':
                ...
                continue
            end_time = time.time()
            computation_times[f"{model_type}_{num_topics}"] = end_time - start_time
        except Exception as e:
            logging.error(f"Error processing {domain} - {dataset_name} - {model_type}: {str(e)}")
            continue

    return metrics_list, computation_times
...
def main():
    ...
    for domain, domain_datasets in datasets.items():
        for dataset_name, data in domain_datasets.items():
            metrics, computation_times = process_dataset(domain, dataset_name, data, model_types, num_topics_list, tokenizer, bert_model)
            all_metrics.extend(metrics)
            all_computation_times[f"{domain}_{dataset_name}"] = computation_times
    ...
    with open('computation_times.json', 'w') as f:
        json.dump(all_computation_times, f, indent=4)
